...
public class RdeIO {
  ...
  abstract static class Write
    ...
    abstract PGPPublicKey stagingKey();
    ...
    abstract static class Builder {
      ...
      abstract Builder setStagingKey(PGPPublicKey value);
    ...
    public PDone expand(PCollection<KV<PendingDeposit, Iterable<DepositFragment>>> input) {
      input
          .apply(
              "Write to GCS",
              ParDo.of(new RdeWriter(gcsUtils(), rdeBucket(), stagingKey(), validationMode())))
  ...
  private static class RdeWriter
    ...
    private final PGPPublicKey stagingKey;
    private final RdeMarshaller marshaller;

    protected RdeWriter(
        GcsUtils gcsUtils,
        String rdeBucket,
        PGPPublicKey stagingKey,
  ...
  class Read<T> extends PTransform<PBegin, PCollection<VKey<T>>> {
    @Override
    public PCollection<VKey<T>> expand(PBegin input) {
      return null;
    }
  }
}
